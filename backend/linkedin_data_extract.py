...
def parse_all_html(move_files=False, role="", loc=""):
    results = []
    ...
    for file in html_files:
        try:
            ...
            # Title match - only check if role is provided and not empty
            first_role_title = get_first_role_title(find_experience(html))
            if role and role.strip():  # FIXED: Only filter if role is specified
                title_match = fuzzy_match(role, first_role_title)
            else:
                title_match = True  # Accept all if no role filter

            # Location match - only check if loc is provided and not empty
            loc_match = True  # FIXED: Default to True (accept all)
            if loc and loc.strip():  # FIXED: Only filter if location is specified
                loc_match = False
